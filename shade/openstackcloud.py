...
class OpenStackCloud(object):
    ...
    def _neutron_available_floating_ips(
        ...
        :param network: List of Nova pools name or Neutron network names or
                        ids.
        ...
        with _utils.neutron_exceptions("unable to get available floating IPs"):
            if network:
                ...
                for x in network:
                    _filter_network = self.search_networks(x)
                    if _filter_network:
                        filter_network = _filter_network[0]
                        break

                if not filter_network:
                    raise OpenStackCloudResourceNotFound(
                        "Unable to find any of %s external networks"
                        % str(network))

                if 'id' in filter_network:
                    floating_network_id = filter_network['id']
                else:
                    raise OpenStackCloudResourceNotFound(
                        "Unable to find %s external network" % str(network))
            else:
